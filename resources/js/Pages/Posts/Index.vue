...
const postFilter = computed(() => {
    ...
    const filteredPosts = props.posts.data.filter((post) => {
        ...
        const tagMatches = tagSearch.value.length === 0 || post.tags.some(tag => tagSearch.value.includes(tag.name));
...
<template>
    ...
    <AuthenticatedLayout>
      ...
      <div class="container mx-auto py-10 flex flex-col md:flex-row-reverse gap-6">
        <!-- 投稿リスト -->
        <div class="flex-1">
          <ul class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <li v-for="post in postFilter" :key="post.id" class="bg-white rounded-lg shadow-lg overflow-hidden">
              <Link :href="`/posts/${post.id}`" class="block hover:shadow-xl transition-shadow duration-300">
                <div class="p-4">
                  <h2 class="text-xm font-semibold text-gray-900 mb-3">タイトル：{{ post.title }}</h2>
        ...
        <div class="w-full md:w-1/3">
          <div class="hidden md:block bg-indigo-50 rounded-lg shadow p-4">
            ...
            <div class="mb-6">
              ...
              <div v-for="(tags, category) in categories" :key="category" class="mb-4">
                <h4 class="text-base font-semibold mb-2">{{ category }}</h4>
          ...
          <div class="md:hidden">
            <div class="mb-4 flex">
              <button @click="setActiveTab('tags')" :class="activeTab === 'tags' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'" class="px-4 py-2 rounded-l-full w-1/2">タグ検索</button>
              <button @click="setActiveTab('keyword')" :class="activeTab === 'keyword' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-800'" class="px-4 py-2 rounded-r-full w-1/2">キーワード検索</button>
            </div>
            <div v-if="activeTab === 'tags'" class="bg-indigo-50 rounded-lg shadow p-4">
              <h3 class="text-lg font-semibold mb-4">タグ検索</h3>
              <div v-for="(tags, category) in categories" :key="category" class="mb-4">
                <h4 class="text-base font-semibold mb-2">{{ category }}</h4>
                <div class="flex flex-wrap gap-2">
            ...
            <div v-if="activeTab === 'keyword'" class="bg-indigo-50 rounded-lg shadow p-4">
              <h3 class="text-lg font-semibold mb-4">キーワード検索</h3>
              <input v-model="searchQuery" 
                     type="text" 
                     placeholder="キーワードを入力" 
